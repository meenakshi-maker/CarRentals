...
def rNew(vRent, cRent):
    ...
    if cRent.currRental != None:
        print(f"Client currently renting: {cRent.currRental.vYear} {cRent.currRental.vModel}")
    else:
        ...
        vRent.vRenter = cRent
        vRent.vStatus = False
        print(f"{cRent.cFName} {cRent.cLName}: {vRent.vMake} {vRent.vModel} = {cRent.cOwing}")
...
def rPay():
    ...
    for u in cList:
        payMenu.append(f"{u.cFName} {u.cLName} = {u.cOwing}")
